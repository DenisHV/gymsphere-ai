...
class UsuariosService {
  ...
  static Future<List<dynamic>> listar() async {
    final respuesta = await http.get(Uri.parse('$_urlBase/usuarios'), headers: _cabeceras);
  ...
  static Future<void> crear({
    ...
    final respuesta = await http.post(
      ...
      body: jsonEncode({'nombre': nombre, 'correo': correo, 'clave': clave, 'rol': rol}),
    );
  ...
  static Future<void> eliminar(int id) async {
    final respuesta = await http.delete(Uri.parse('$_urlBase/usuarios/$id'), headers: _cabeceras);
    if (respuesta.statusCode != 200) {
  ...
  static Future<void> resetear2FA(int id) async {
    final respuesta = await http.patch(Uri.parse('$_urlBase/usuarios/$id/reset-2fa'), headers: _cabeceras);
    if (respuesta.statusCode != 200) {
